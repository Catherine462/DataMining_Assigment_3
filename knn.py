...
df_data = pd.read_csv('weather_training.csv')

# separating feature values and label
X_data = df_data.iloc[:, 1:].values  
y_data = df_data.iloc[:, -1].values

for i in range(len(X_data)):
    if isinstance(X_data[i][0], str):
        date_str = X_data[i][0].split()[0]  
        year = int(date_str)  
        X_data[i][0] = year  
...
textual_data = np.concatenate([X_data[:, df_data.columns.get_loc(feature)].reshape(-1, 1) for feature in textual_features], axis=1)
encoder.fit(textual_data.flatten())

for i, feature in enumerate(textual_features):
    X_data[:, df_data.columns.get_loc(feature)] = encoder.transform(X_data[:, df_data.columns.get_loc(feature)])

# scaling numerical feature
scaler = StandardScaler()
X_data[:, len(textual_features):] = scaler.fit_transform(X_data[:, len(textual_features):])

# 11 classes after discretization
classes = np.array([i for i in range(-22, 40, 6)])
y_data_discretized = np.digitize(y_data, classes)
...
grid_search.fit(X_data, y_data_discretized)
