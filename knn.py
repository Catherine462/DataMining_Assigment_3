...
X_training = df_train.iloc[:, 1:].values  
y_training = df_train.iloc[:, -1].values
...
textual_data = np.concatenate([X_training[:, df_train.columns.get_loc(feature)].reshape(-1, 1) for feature in textual_features], axis=1)
encoder.fit(textual_data.flatten())

for i, feature in enumerate(textual_features):
    X_training[:, df_train.columns.get_loc(feature)] = encoder.transform(X_training[:, df_train.columns.get_loc(feature)])
